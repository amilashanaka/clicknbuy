...
$form_config = $allergy_page_elements;
if (isset($_GET['id'])) { $id = base64_decode($_GET['id']); $row = $log->get_by_id($id)['data']; } else { $id = 0; $row = null; }
?>
                            ...
                            <div>
                                <form action="<?= $form_config['form_action'] ?>" class="templatemo-login-form" method="post" enctype="multipart/form-data" name="update_vehicles">
                                    ...
                                    if ($id == 0) {
                                        ...
                                        echo '<input type="hidden" name="created_date" value="' . $today. '">';
                                    } else {
                                        echo '<input type="hidden" name="update_by" value="' . $user_act . '">';
                                        echo '<input type="hidden" name="update_date" value="' . $user_act . '">';
                                    }
                                    ?>
