...
$wanted_keys = ['id', 'f1', 'created_by', 'updated_by', 'created_date', 'updated_date'];
...
if ($id <= 0) {
    $result = $allergy->register($data);
    if ($result['code'] == 200) {
        header('Location: ../allergies_list' . '?error=' . base64_encode(4));
    } else {
        header('Location: ../allergy?id=' . base64_encode($id) . '&error=' . base64_encode(3));
    }
} else {
