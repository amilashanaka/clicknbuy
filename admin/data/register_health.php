...
if ($id <= 0) {
    ...
    $result = $health->register($data);


    if ($result['code'] == 200) {
        header('Location: ../health_list' . '?error=' . base64_encode(4));
    } else {
        header('Location: ../health?id=' . base64_encode($id) . '&error=' . base64_encode(3));
    }
} else {



    $result = $health->update($data);
    ...
    if ($result['error'] == null) {
        ...
        if ($result['status'] == 1) {
            ...
            $info = implode(" ", $info);
            header('Location: ../health_list?error=' . base64_encode(1) . '&info=' . base64_encode($info));
        } else {
            header('Location: ../health?id=' . base64_encode($id) . '&error=' . base64_encode(2) . '&info=' . base64_encode($info));
        }
    } else {
